...
function Truck(callback) {
  var me = this;

  
  me.TRAILING_DISTANCE = 500;
  me.TILT = 80;
  me.CAM_HEIGHT = 0;
  ...
  me.enabled = false;
  me.follow = false;
  me.forwardonly = false;
  me.canturn = true;
  me.canacceldecel = true;
  
  me.constantheadinglat = 0;
  me.constantheadinglng = 0;
  me.constantheading = false;
  ...
  me.optionaltick = null;
  
  ge.getOptions().setMouseNavigationEnabled(false);
  ge.getOptions().setFlyToSpeed(100);  // don't filter camera motion

  window.google.earth.fetchKml(ge, MODEL_URL, function(obj) { me.finishInit(obj, callback); });
 
}

Truck.prototype.finishInit = function(kml, callback) {
  ...
  walkKmlDom(kml, function() {
    if (this.getType() == 'KmlPlacemark' &&
      ...
      me.placemark = this;
    //else
    	//setProgress(10, "Failed Loading Rover");
  });
  // moved this from the walk function
 // me.placemark = this;
  ...
  //setProgress(10, "Configuring1 Rover");
  
  ge.getFeatures().appendChild(me.placemark);

  me.balloon = ge.createHtmlStringBalloon(randomId());
  ...
  me.shadow = ge.createGroundOverlay(randomId());
  me.shadow.setVisibility(false);
  me.shadow.setIcon(ge.createIcon(randomId()));
  me.shadow.setLatLonBox(ge.createLatLonBox(randomId()));
...
Truck.prototype.lockOn = function() {
		var me = this;
...
Truck.prototype.tick = function(callback) {
  ...
  if (me.constantheading){
	  ...
	  var lookAtPoint = new geo.Point(
		  ...
		  var vantageView = gex.view.createVantageView(cameraPoint, lookAtPoint); 
	  
		var truckHeading = truck.model.getOrientation().getHeading();
		var camHeading = vantageView.getHeading();
		
		var deltaHeading = 0.01 * fixAngle(truckHeading - camHeading);

		//setDebug(deltaHeading);
		  
		steerAngle = deltaHeading;
  }
  ...
  if (!airborne) {
    ...
    //setDebug(forwardSpeed);
    
    if (me.canacceldecel && gasButtonDown) {
      // Accelerate forwards.
      me.vel = V3.add(me.vel, V3.scale(dir, ACCEL * dt));
    } else if ( me.canacceldecel && reverseButtonDown) {
      if (forwardSpeed > -MAX_REVERSE_SPEED)
        me.vel = V3.add(me.vel, V3.scale(dir, -DECEL * dt));
    }
    if (me.forwardonly && forwardSpeed < 0)
    	me.vel = V3.add(me.vel, V3.scale(dir, ACCEL * dt));
  ...
  me.NORMAL = estimateGroundNormal(gpos, me.localFrame);
  ...
  if (!airborne) {
    ...
    var speedOutOfGround = V3.dot(me.NORMAL, me.vel);
    if (speedOutOfGround < 0) {
      me.vel = V3.add(me.vel, V3.scale(me.NORMAL, -speedOutOfGround));
    }
...
Truck.prototype.showPopup = function(m) {
	  var me = this;
	  me.popupTimer = 5.0;
	  var message = "<center>" + m + "</center>";
	  me.balloon.setContentString(message);
	  ge.setBalloon(me.balloon);
	};
	
Truck.prototype.showPopupLong = function(m) {
	  var me = this;
	  me.popupTimer = 500.0;
	  var message = "<center>" + m + "</center>";
	  me.balloon.setContentString(message);
...
Truck.prototype.showFastPopup = function(m) {
  var me = this;
  me.popupTimer = 2.0;
  var rand = Math.random();
...
Truck.prototype.cameraCut = function() {
  ...
  var la = ge.createLookAt(randomId());
...
Truck.prototype.cameraFollow = function(dt, truckPos, localToGlobalFrame) {
  ...
  var headingDir = V3.rotate(localToGlobalFrame[1], localToGlobalFrame[2],
                             -headingRadians);
  var camPos = V3.add(truckPos, V3.scale(localToGlobalFrame[2], me.CAM_HEIGHT));
  camPos = V3.add(camPos, V3.scale(headingDir, -me.TRAILING_DISTANCE));
  ...
  la.set(camLat, camLon, camAlt, ge.ALTITUDE_RELATIVE_TO_SEA_FLOOR, 
        heading, me.TILT /*tilt*/, 0 /*range*/);
  ge.getOptions().setFlyToSpeed(me.followspeed);
  ge.getView().setAbstractView(la);
};
